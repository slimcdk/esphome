...
void TMC2226Stepper::dump_config() {
  ESP_LOGCONFIG(TAG, "TMC2226 Stepper:");
  LOG_STEPPER(this);
  LOG_TMC2226(this);
}

void TMC2226Stepper::setup() {
  ESP_LOGCONFIG(TAG, "Setting up TMC2226 Stepper...");
  TMC2226Component::setup();
  ...
  ESP_LOGCONFIG(TAG, "TMC2226 Stepper setup done.");
}

void TMC2226Stepper::on_shutdown() { this->stop(); }

void TMC2226Stepper::loop() {
  TMC2226Component::loop();
...
void TMC2226Stepper::set_target(int32_t steps) {
...
void TMC2226Stepper::enable(bool enable) {
  if (!enable) {
    this->stop();
  }
  TMC2226Component::enable(enable);
}

bool TMC2226Stepper::is_stalled() {
