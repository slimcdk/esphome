...
from esphome.components import tmc2226_hub
...
AUTO_LOAD = ["tmc2226_hub"]

CONF_TMC2226 = "tmc2226"
CONF_TMC2226_ID = "tmc2226_id"
...
tmc2226_ns = cg.esphome_ns.namespace("tmc2226")
TMC2226API = tmc2226_ns.class_(
    "TMC2226API", cg.Parented.template(tmc2226_hub.TMC2226Hub)
)
TMC2226Component = tmc2226_ns.class_("TMC2226Component", TMC2226API, cg.Component)

DriverStatusEvent = tmc2226_ns.enum("DriverStatusEvent")
StandstillMode = tmc2226_ns.enum("StandstillMode")
ShaftDirection = tmc2226_ns.enum("ShaftDirection")

OnDriverStatusTrigger = tmc2226_ns.class_("OnDriverStatusTrigger", automation.Trigger)
OnStallTrigger = tmc2226_ns.class_("OnStallTrigger", automation.Trigger)

ConfigureAction = tmc2226_ns.class_("ConfigureAction", automation.Action)
ActivationAction = tmc2226_ns.class_("ActivationAction", automation.Action)
CurrentsAction = tmc2226_ns.class_("CurrentsAction", automation.Action)
StallGuardAction = tmc2226_ns.class_("StallGuardAction", automation.Action)
CoolConfAction = tmc2226_ns.class_("CoolConfAction", automation.Action)
ChopConfAction = tmc2226_ns.class_("ChopConfAction", automation.Action)
PWMConfAction = tmc2226_ns.class_("PWMConfAction", automation.Action)
SyncAction = tmc2226_ns.class_("SyncAction", automation.Action)
...
DEVICE_SCHEMA = cv.Schema({cv.GenerateID(CONF_TMC2226_ID): cv.use_id(TMC2226Component)})

TMC2226_BASE_CONFIG_SCHEMA = cv.Schema(
    {
...
).extend(cv.COMPONENT_SCHEMA, tmc2226_hub.TMC2226_HUB_DEVICE_SCHEMA)


async def register_tmc2226_base(var, config):
    await cg.register_component(var, config)
    await tmc2226_hub.register_tmc2226_hub_device(var, config)
...
def validate_tmc2226_base(config):
    return config
...
@automation.register_action(
    ...
    maybe_simple_id(
        {
            cv.GenerateID(): cv.use_id(TMC2226Component),
            cv.Optional(CONF_RESTORE_TOFF, default=True): cv.boolean,
        }
    ),
)
async def tmc2226_enable_to_code(config, action_id, template_arg, args):
...
async def tmc2226_disable_to_code(config, action_id, template_arg, args):
    var = cg.new_Pvariable(action_id, template_arg)
    await cg.register_parented(var, config[CONF_ID])
...
async def tmc2226_configure_to_code(config, action_id, template_arg, args):
    var = cg.new_Pvariable(action_id, template_arg)
    await cg.register_parented(var, config[CONF_ID])
...
async def tmc2226_currents_to_code(config, action_id, template_arg, args):
    var = cg.new_Pvariable(action_id, template_arg)
    await cg.register_parented(var, config[CONF_ID])
...
async def tmc2226_stallguard_to_code(config, action_id, template_arg, args):
    var = cg.new_Pvariable(action_id, template_arg)
    await cg.register_parented(var, config[CONF_ID])
...
async def tmc2226_coolconf_to_code(config, action_id, template_arg, args):
    var = cg.new_Pvariable(action_id, template_arg)
    await cg.register_parented(var, config[CONF_ID])
...
async def tmc2226_chopconf_to_code(config, action_id, template_arg, args):
    var = cg.new_Pvariable(action_id, template_arg)
    await cg.register_parented(var, config[CONF_ID])
...
async def tmc2226_pwmconf_to_code(config, action_id, template_arg, args):
    var = cg.new_Pvariable(action_id, template_arg)
    await cg.register_parented(var, config[CONF_ID])
    ...
    cv.Schema(
        {
            ...
            cv.Required(CONF_TO): cv.All(
                cv.ensure_list(cv.use_id(TMC2226Component)), cv.Length(min=1)
            ),
        }
    ),
)
async def tmc2226_sync_to_code(config, action_id, template_arg, args):
    var = cg.new_Pvariable(action_id, template_arg)
    await cg.register_parented(var, config[CONF_ID])

    if config[CONF_ID] in config[CONF_TO]:
        _LOGGER.error("tmc2226.sync for %s is syncing to self", config[CONF_ID])

    if len(config[CONF_TO]) != len(set(config[CONF_TO])):
        _LOGGER.warning("tmc2226.sync for %s has duplicate references", config[CONF_ID])

    template_ = await cg.templatable(
        [await cg.get_variable(id) for id in config[CONF_TO]],
        args,
        cg.std_vector.template(TMC2226Component),
    )
