...
from .. import (
    ...
    CONF_STEP_PIN,
    CONF_TMC2225_ID,
    TMC2225_BASE_CONFIG_SCHEMA,
    TMC2225Component,
    register_tmc2225_base,
    tmc2225_ns,
    validate_tmc2225_base,
)

CODEOWNERS = ["@slimcdk"]

AUTO_LOAD = ["tmc2225_hub", "tmc2225"]

TMC2225Stepper = tmc2225_ns.class_("TMC2225Stepper", TMC2225Component, stepper.Stepper)
ControlMethod = tmc2225_ns.enum("ControlMethod")


DEVICE_SCHEMA = cv.Schema(
    {
        cv.GenerateID(CONF_TMC2225_ID): cv.use_id(TMC2225Stepper),
    }
)
...
CONFIG_SCHEMA = cv.All(
    cv.Schema(
        {
            cv.GenerateID(CONF_ID): cv.declare_id(TMC2225Stepper),
        }
    ).extend(TMC2225_BASE_CONFIG_SCHEMA, stepper.STEPPER_SCHEMA),
...
async def to_code(config):
    ...
    await register_tmc2225_base(var, config)
