...
void TMC2225Stepper::dump_config() {
  ESP_LOGCONFIG(TAG, "TMC2225 Stepper:");
  LOG_STEPPER(this);
  LOG_TMC2225(this);
}

void TMC2225Stepper::setup() {
  ESP_LOGCONFIG(TAG, "Setting up TMC2225 Stepper...");
  TMC2225Component::setup();
  ...
  ESP_LOGCONFIG(TAG, "TMC2225 Stepper setup done.");
}

void TMC2225Stepper::on_shutdown() { this->stop(); }

void TMC2225Stepper::loop() {
  TMC2225Component::loop();
...
void TMC2225Stepper::set_target(int32_t steps) {
...
void TMC2225Stepper::enable(bool enable) {
  if (!enable) {
    this->stop();
  }
  TMC2225Component::enable(enable);
}

bool TMC2225Stepper::is_stalled() {
