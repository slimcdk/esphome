#include "tmc2202_api_registers.h"
#include "tmc2202_component.h"
...
void TMC2202Component::setup() {
  ESP_LOGCONFIG(TAG, "Setting up TMC2202 Component...");
  ...
  ESP_LOGCONFIG(TAG, "TMC2202 Component setup done.");
}

void TMC2202Component::loop() {
...
uint16_t TMC2202Component::get_microsteps() { return MRES_TO_MS(this->read_field(MRES_FIELD)); }
void TMC2202Component::set_microsteps(uint16_t ms) {
...
float TMC2202Component::read_vsense() { return (this->read_field(VSENSE_FIELD) ? 0.180f : 0.325f); }

uint16_t TMC2202Component::current_scale_to_rms_current_mA(uint8_t cs) {
...
uint8_t TMC2202Component::rms_current_to_current_scale_mA_no_clamp(uint16_t mA) {
  return 32.0f * sqrtf(2) * FROM_MILLI(mA) * ((this->rsense_.value_or(0.170) + 0.02f) / this->read_vsense()) - 1.0f;
}

uint8_t TMC2202Component::rms_current_to_current_scale_mA(uint16_t mA) {
  if (mA == 0) {
    return 0;
  }
...
// void TMC2202Component::enable(bool enable, bool recover_toff = true) {
void TMC2202Component::enable(bool enable) {
  if (this->enn_pin_ != nullptr) {
