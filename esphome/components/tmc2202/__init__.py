...
from esphome.components import tmc2202_hub
...
AUTO_LOAD = ["tmc2202_hub"]

CONF_TMC2202 = "tmc2202"
CONF_TMC2202_ID = "tmc2202_id"
...
tmc2202_ns = cg.esphome_ns.namespace("tmc2202")
TMC2202API = tmc2202_ns.class_(
    "TMC2202API", cg.Parented.template(tmc2202_hub.TMC2202Hub)
)
TMC2202Component = tmc2202_ns.class_("TMC2202Component", TMC2202API, cg.Component)

DriverStatusEvent = tmc2202_ns.enum("DriverStatusEvent")
StandstillMode = tmc2202_ns.enum("StandstillMode")
ShaftDirection = tmc2202_ns.enum("ShaftDirection")

OnDriverStatusTrigger = tmc2202_ns.class_("OnDriverStatusTrigger", automation.Trigger)
OnStallTrigger = tmc2202_ns.class_("OnStallTrigger", automation.Trigger)

ConfigureAction = tmc2202_ns.class_("ConfigureAction", automation.Action)
ActivationAction = tmc2202_ns.class_("ActivationAction", automation.Action)
CurrentsAction = tmc2202_ns.class_("CurrentsAction", automation.Action)
StallGuardAction = tmc2202_ns.class_("StallGuardAction", automation.Action)
CoolConfAction = tmc2202_ns.class_("CoolConfAction", automation.Action)
ChopConfAction = tmc2202_ns.class_("ChopConfAction", automation.Action)
PWMConfAction = tmc2202_ns.class_("PWMConfAction", automation.Action)
SyncAction = tmc2202_ns.class_("SyncAction", automation.Action)
...
DEVICE_SCHEMA = cv.Schema({cv.GenerateID(CONF_TMC2202_ID): cv.use_id(TMC2202Component)})

TMC2202_BASE_CONFIG_SCHEMA = cv.Schema(
    {
...
).extend(cv.COMPONENT_SCHEMA, tmc2202_hub.TMC2202_HUB_DEVICE_SCHEMA)


async def register_tmc2202_base(var, config):
    await cg.register_component(var, config)
    await tmc2202_hub.register_tmc2202_hub_device(var, config)
...
def validate_tmc2202_base(config):
    return config
...
@automation.register_action(
    ...
    maybe_simple_id(
        {
            cv.GenerateID(): cv.use_id(TMC2202Component),
            cv.Optional(CONF_RESTORE_TOFF, default=True): cv.boolean,
        }
    ),
)
async def tmc2202_enable_to_code(config, action_id, template_arg, args):
...
async def tmc2202_disable_to_code(config, action_id, template_arg, args):
    var = cg.new_Pvariable(action_id, template_arg)
    await cg.register_parented(var, config[CONF_ID])
...
async def tmc2202_configure_to_code(config, action_id, template_arg, args):
    var = cg.new_Pvariable(action_id, template_arg)
    await cg.register_parented(var, config[CONF_ID])
...
async def tmc2202_currents_to_code(config, action_id, template_arg, args):
    var = cg.new_Pvariable(action_id, template_arg)
    await cg.register_parented(var, config[CONF_ID])
...
async def tmc2202_chopconf_to_code(config, action_id, template_arg, args):
    var = cg.new_Pvariable(action_id, template_arg)
    await cg.register_parented(var, config[CONF_ID])
...
async def tmc2202_pwmconf_to_code(config, action_id, template_arg, args):
    var = cg.new_Pvariable(action_id, template_arg)
    await cg.register_parented(var, config[CONF_ID])
    ...
    cv.Schema(
        {
            ...
            cv.Required(CONF_TO): cv.All(
                cv.ensure_list(cv.use_id(TMC2202Component)), cv.Length(min=1)
            ),
        }
    ),
)
async def tmc2202_sync_to_code(config, action_id, template_arg, args):
    var = cg.new_Pvariable(action_id, template_arg)
    await cg.register_parented(var, config[CONF_ID])

    if config[CONF_ID] in config[CONF_TO]:
        _LOGGER.error("tmc2202.sync for %s is syncing to self", config[CONF_ID])

    if len(config[CONF_TO]) != len(set(config[CONF_TO])):
        _LOGGER.warning("tmc2202.sync for %s has duplicate references", config[CONF_ID])

    template_ = await cg.templatable(
        [await cg.get_variable(id) for id in config[CONF_TO]],
        args,
        cg.std_vector.template(TMC2202Component),
    )
