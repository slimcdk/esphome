...
void TMC2202Stepper::dump_config() {
  ESP_LOGCONFIG(TAG, "TMC2202 Stepper:");
  LOG_STEPPER(this);
  LOG_TMC2202(this);
}

void TMC2202Stepper::setup() {
  ESP_LOGCONFIG(TAG, "Setting up TMC2202 Stepper...");
  TMC2202Component::setup();
  ...
  ESP_LOGCONFIG(TAG, "TMC2202 Stepper setup done.");
}

void TMC2202Stepper::on_shutdown() { this->stop(); }

void TMC2202Stepper::loop() {
  TMC2202Component::loop();
...
void TMC2202Stepper::set_target(int32_t steps) {
...
void TMC2202Stepper::enable(bool enable) {
  if (!enable) {
    this->stop();
  }
  TMC2202Component::enable(enable);
}
