...
void TMCStepper::setup() {
  ESP_LOGCONFIG(this->model_, "Setting up TMCComponent...");
  ...
  ESP_LOGCONFIG(this->model_, "TMCComponent setup done.");
}

void TMCStepper::dump_config() {
  ESP_LOGCONFIG(this->model_, "Stepper:");
  LOG_STEPPER(this);
}

void TMCStepper::loop() {
...
uint16_t TMCStepper::get_microsteps() { return MRES_TO_MS(this->read_field(MRES_FIELD)); }
void TMCStepper::set_microsteps(uint16_t ms) {
  ...
  ESP_LOGW(this->model_, "%d is not a valid microstepping option", ms);
}

float TMCStepper::get_motor_load() {
  const int32_t result = this->read_register(SG_RESULT);
  return (510.0 - (float) result) / (510.0 - (int32_t) this->read_register(SGTHRS) * 2.0);
}

float TMCStepper::read_vsense() { return (this->read_field(VSENSE_FIELD) ? 0.180f : 0.325f); }

uint16_t TMCStepper::current_scale_to_rms_current_mA(uint8_t cs) {
...
uint8_t TMCStepper::rms_current_to_current_scale_mA_no_clamp(uint16_t mA) {
  return 32.0f * sqrtf(2) * FROM_MILLI(mA) * ((this->rsense_.value_or(0.170) + 0.02f) / this->read_vsense()) - 1.0f;
}

uint8_t TMCStepper::rms_current_to_current_scale_mA(uint16_t mA) {
  if (mA == 0) {
    return 0;
  }
  ...
  if (cs > 31) {
    ...
    ESP_LOGW(this->model_, "Configured RSense and VSense has a max current limit of %d mA. Clamping value to max!",
             mA_limit);
  }
...
void TMCStepper::on_shutdown() { this->stop(); }

uint32_t TMCStepper::get_tpowerdown_ms() {
  return (this->read_field(TPOWERDOWN_FIELD) * 262144.0) / (this->clk_freq_ / 1000.0);
};

std::tuple<uint8_t, uint8_t> TMCStepper::unpack_ottrim_values(uint8_t ottrim) {
  switch (ottrim) {
    case 0b00:
...
// void TMCStepper::enable(bool enable, bool recover_toff = true) {
void TMCStepper::enable(bool enable) {
  if (!enable) {
    this->stop();
  }
