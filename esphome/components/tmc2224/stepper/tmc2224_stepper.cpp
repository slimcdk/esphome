...
void TMC2224Stepper::dump_config() {
  ESP_LOGCONFIG(TAG, "TMC2224 Stepper:");
  LOG_STEPPER(this);
  LOG_TMC2224(this);
}

void TMC2224Stepper::setup() {
  ESP_LOGCONFIG(TAG, "Setting up TMC2224 Stepper...");
  TMC2224Component::setup();
  ...
  ESP_LOGCONFIG(TAG, "TMC2224 Stepper setup done.");
}

void TMC2224Stepper::on_shutdown() { this->stop(); }

void TMC2224Stepper::loop() {
  TMC2224Component::loop();
...
void TMC2224Stepper::set_target(int32_t steps) {
...
void TMC2224Stepper::enable(bool enable) {
  if (!enable) {
    this->stop();
  }
  TMC2224Component::enable(enable);
}
