...
void TMC2224Component::setup() {
  ESP_LOGCONFIG(TAG, "Setting up TMC2224 Component...");
  ...
  ESP_LOGCONFIG(TAG, "TMC2224 Component setup done.");
}

void TMC2224Component::loop() {
...
uint16_t TMC2224Component::get_microsteps() { return MRES_TO_MS(this->read_field(MRES_FIELD)); }
void TMC2224Component::set_microsteps(uint16_t ms) {
...
float TMC2224Component::read_vsense() { return (this->read_field(VSENSE_FIELD) ? 0.180f : 0.325f); }

uint16_t TMC2224Component::current_scale_to_rms_current_mA(uint8_t cs) {
...
uint8_t TMC2224Component::rms_current_to_current_scale_mA_no_clamp(uint16_t mA) {
  return 32.0f * sqrtf(2) * FROM_MILLI(mA) * ((this->rsense_.value_or(0.170) + 0.02f) / this->read_vsense()) - 1.0f;
}

uint8_t TMC2224Component::rms_current_to_current_scale_mA(uint16_t mA) {
  if (mA == 0) {
    return 0;
  }
...
// void TMC2224Component::enable(bool enable, bool recover_toff = true) {
void TMC2224Component::enable(bool enable) {
  if (this->enn_pin_ != nullptr) {
