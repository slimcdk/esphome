...
from esphome.components import tmc2224_hub
...
AUTO_LOAD = ["tmc2224_hub"]

CONF_TMC2224 = "tmc2224"
CONF_TMC2224_ID = "tmc2224_id"
...
tmc2224_ns = cg.esphome_ns.namespace("tmc2224")
TMC2224API = tmc2224_ns.class_(
    "TMC2224API", cg.Parented.template(tmc2224_hub.TMC2224Hub)
)
TMC2224Component = tmc2224_ns.class_("TMC2224Component", TMC2224API, cg.Component)

DriverStatusEvent = tmc2224_ns.enum("DriverStatusEvent")
StandstillMode = tmc2224_ns.enum("StandstillMode")
ShaftDirection = tmc2224_ns.enum("ShaftDirection")

OnDriverStatusTrigger = tmc2224_ns.class_("OnDriverStatusTrigger", automation.Trigger)
OnStallTrigger = tmc2224_ns.class_("OnStallTrigger", automation.Trigger)

ConfigureAction = tmc2224_ns.class_("ConfigureAction", automation.Action)
ActivationAction = tmc2224_ns.class_("ActivationAction", automation.Action)
CurrentsAction = tmc2224_ns.class_("CurrentsAction", automation.Action)
StallGuardAction = tmc2224_ns.class_("StallGuardAction", automation.Action)
CoolConfAction = tmc2224_ns.class_("CoolConfAction", automation.Action)
ChopConfAction = tmc2224_ns.class_("ChopConfAction", automation.Action)
PWMConfAction = tmc2224_ns.class_("PWMConfAction", automation.Action)
SyncAction = tmc2224_ns.class_("SyncAction", automation.Action)
...
DEVICE_SCHEMA = cv.Schema({cv.GenerateID(CONF_TMC2224_ID): cv.use_id(TMC2224Component)})

TMC2224_BASE_CONFIG_SCHEMA = cv.Schema(
    {
...
).extend(cv.COMPONENT_SCHEMA, tmc2224_hub.TMC2224_HUB_DEVICE_SCHEMA)


async def register_tmc2224_base(var, config):
    await cg.register_component(var, config)
    await tmc2224_hub.register_tmc2224_hub_device(var, config)
...
def validate_tmc2224_base(config):
    return config
...
@automation.register_action(
    ...
    maybe_simple_id(
        {
            cv.GenerateID(): cv.use_id(TMC2224Component),
            cv.Optional(CONF_RESTORE_TOFF, default=True): cv.boolean,
        }
    ),
)
async def tmc2224_enable_to_code(config, action_id, template_arg, args):
...
async def tmc2224_disable_to_code(config, action_id, template_arg, args):
    var = cg.new_Pvariable(action_id, template_arg)
    await cg.register_parented(var, config[CONF_ID])
...
async def tmc2224_configure_to_code(config, action_id, template_arg, args):
    var = cg.new_Pvariable(action_id, template_arg)
    await cg.register_parented(var, config[CONF_ID])
...
async def tmc2224_currents_to_code(config, action_id, template_arg, args):
    var = cg.new_Pvariable(action_id, template_arg)
    await cg.register_parented(var, config[CONF_ID])
...
async def tmc2224_chopconf_to_code(config, action_id, template_arg, args):
    var = cg.new_Pvariable(action_id, template_arg)
    await cg.register_parented(var, config[CONF_ID])
...
async def tmc2224_pwmconf_to_code(config, action_id, template_arg, args):
    var = cg.new_Pvariable(action_id, template_arg)
    await cg.register_parented(var, config[CONF_ID])
    ...
    cv.Schema(
        {
            ...
            cv.Required(CONF_TO): cv.All(
                cv.ensure_list(cv.use_id(TMC2224Component)), cv.Length(min=1)
            ),
        }
    ),
)
async def tmc2224_sync_to_code(config, action_id, template_arg, args):
    var = cg.new_Pvariable(action_id, template_arg)
    await cg.register_parented(var, config[CONF_ID])

    if config[CONF_ID] in config[CONF_TO]:
        _LOGGER.error("tmc2224.sync for %s is syncing to self", config[CONF_ID])

    if len(config[CONF_TO]) != len(set(config[CONF_TO])):
        _LOGGER.warning("tmc2224.sync for %s has duplicate references", config[CONF_ID])

    template_ = await cg.templatable(
        [await cg.get_variable(id) for id in config[CONF_TO]],
        args,
        cg.std_vector.template(TMC2224Component),
    )
