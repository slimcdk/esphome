...
CONF_TMC_BASE = "tmc_base"
CONF_TMC_BASE_ID = "tmc_base_id"
...
tmc_base_ns = cg.esphome_ns.namespace("tmc_base")
TMCBaseAPI = tmc_base_ns.class_(
    "TMCBaseAPI", cg.Parented.template(tmc_hub.TMCBaseHub)
)
TMCBaseComponent = tmc_base_ns.class_("TMCBaseComponent", TMCBaseAPI, cg.Component)

DriverStatusEvent = tmc_base_ns.enum("DriverStatusEvent")
StandstillMode = tmc_base_ns.enum("StandstillMode")
ShaftDirection = tmc_base_ns.enum("ShaftDirection")

OnDriverStatusTrigger = tmc_base_ns.class_("OnDriverStatusTrigger", automation.Trigger)
OnStallTrigger = tmc_base_ns.class_("OnStallTrigger", automation.Trigger)

ConfigureAction = tmc_base_ns.class_("ConfigureAction", automation.Action)
ActivationAction = tmc_base_ns.class_("ActivationAction", automation.Action)
CurrentsAction = tmc_base_ns.class_("CurrentsAction", automation.Action)
StallGuardAction = tmc_base_ns.class_("StallGuardAction", automation.Action)
CoolConfAction = tmc_base_ns.class_("CoolConfAction", automation.Action)
ChopConfAction = tmc_base_ns.class_("ChopConfAction", automation.Action)
PWMConfAction = tmc_base_ns.class_("PWMConfAction", automation.Action)
SyncAction = tmc_base_ns.class_("SyncAction", automation.Action)
...
DEVICE_SCHEMA = cv.Schema({cv.GenerateID(CONF_TMC_BASE_ID): cv.use_id(TMCBaseComponent)})

TMCBase_BASE_CONFIG_SCHEMA = cv.Schema(
    {
...
).extend(cv.COMPONENT_SCHEMA, tmc_hub.TMCBase_HUB_DEVICE_SCHEMA)


async def register_tmc_base_base(var, config):
...
def validate_tmc_base_base(config):
    return config
...
@automation.register_action(
    ...
    maybe_simple_id(
        {
            cv.GenerateID(): cv.use_id(TMCBaseComponent),
            cv.Optional(CONF_RESTORE_TOFF, default=True): cv.boolean,
        }
    ),
)
async def tmc_base_enable_to_code(config, action_id, template_arg, args):
...
async def tmc_base_disable_to_code(config, action_id, template_arg, args):
    var = cg.new_Pvariable(action_id, template_arg)
    await cg.register_parented(var, config[CONF_ID])
...
async def tmc_base_configure_to_code(config, action_id, template_arg, args):
    var = cg.new_Pvariable(action_id, template_arg)
    await cg.register_parented(var, config[CONF_ID])
...
async def tmc_base_currents_to_code(config, action_id, template_arg, args):
    var = cg.new_Pvariable(action_id, template_arg)
    await cg.register_parented(var, config[CONF_ID])
...
async def tmc_base_stallguard_to_code(config, action_id, template_arg, args):
    var = cg.new_Pvariable(action_id, template_arg)
    await cg.register_parented(var, config[CONF_ID])
...
async def tmc_base_coolconf_to_code(config, action_id, template_arg, args):
    var = cg.new_Pvariable(action_id, template_arg)
    await cg.register_parented(var, config[CONF_ID])
...
async def tmc_base_chopconf_to_code(config, action_id, template_arg, args):
    var = cg.new_Pvariable(action_id, template_arg)
    await cg.register_parented(var, config[CONF_ID])
...
async def tmc_base_pwmconf_to_code(config, action_id, template_arg, args):
    var = cg.new_Pvariable(action_id, template_arg)
    await cg.register_parented(var, config[CONF_ID])
    ...
    cv.Schema(
        {
            ...
            cv.Required(CONF_TO): cv.All(
                cv.ensure_list(cv.use_id(TMCBaseComponent)), cv.Length(min=1)
            ),
        }
    ),
)
async def tmc_base_sync_to_code(config, action_id, template_arg, args):

    var = cg.new_Pvariable(action_id, template_arg)
    await cg.register_parented(var, config[CONF_ID])

    if config[CONF_ID] in config[CONF_TO]:
        _LOGGER.error("tmc_base.sync for %s is syncing to self", config[CONF_ID])

    if len(config[CONF_TO]) != len(set(config[CONF_TO])):
        _LOGGER.warning("tmc_base.sync for %s has duplicate references", config[CONF_ID])

    template_ = await cg.templatable(
        [await cg.get_variable(id) for id in config[CONF_TO]],
        args,
        cg.std_vector.template(TMCBaseComponent),
    )
