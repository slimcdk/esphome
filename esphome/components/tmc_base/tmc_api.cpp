...
uint8_t TMCBaseAPI::crc8_(uint8_t *data, uint32_t bytes) {
  uint8_t result = 0;
  uint8_t *table;
...
void TMCBaseAPI::set_dirty_bit_(uint8_t index, bool value) {
...
bool TMCBaseAPI::get_dirty_bit_(uint8_t index) {
  if (index >= REGISTER_COUNT)
    return false;
...
bool TMCBaseAPI::cache_(CacheOperation operation, uint8_t address, uint32_t *value) {
...
void TMCBaseAPI::write_register(uint8_t address, int32_t value) {
...
int32_t TMCBaseAPI::read_register(uint8_t address) {
...
uint32_t TMCBaseAPI::update_field(uint32_t data, RegisterField field, uint32_t value) {
  return (data & (~field.mask)) | ((value << field.shift) & field.mask);
}

void TMCBaseAPI::write_field(RegisterField field, uint32_t value) {
...
uint32_t TMCBaseAPI::extract_field(uint32_t data, RegisterField field) {
...
uint32_t TMCBaseAPI::read_field(RegisterField field) {
