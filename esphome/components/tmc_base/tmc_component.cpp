#include "tmc_base_api_registers.h"
#include "tmc_base_component.h"
...
void TMCBaseComponent::setup() {
  ESP_LOGCONFIG(TAG, "Setting up TMCBase Component...");
  ...
  ESP_LOGCONFIG(TAG, "TMCBase Component setup done.");
}

void TMCBaseComponent::loop() {
...
bool TMCBaseComponent::is_stalled() {
...
uint16_t TMCBaseComponent::get_microsteps() { return MRES_TO_MS(this->read_field(MRES_FIELD)); }
void TMCBaseComponent::set_microsteps(uint16_t ms) {
...
float TMCBaseComponent::get_motor_load() {
  const int32_t result = this->read_register(SG_RESULT);
  return (510.0 - (float) result) / (510.0 - (int32_t) this->read_register(SGTHRS) * 2.0);
}

float TMCBaseComponent::read_vsense() { return (this->read_field(VSENSE_FIELD) ? 0.180f : 0.325f); }

uint16_t TMCBaseComponent::current_scale_to_rms_current_mA(uint8_t cs) {
...
uint8_t TMCBaseComponent::rms_current_to_current_scale_mA_no_clamp(uint16_t mA) {
  return 32.0f * sqrtf(2) * FROM_MILLI(mA) * ((this->rsense_.value_or(0.170) + 0.02f) / this->read_vsense()) - 1.0f;
}

uint8_t TMCBaseComponent::rms_current_to_current_scale_mA(uint16_t mA) {
  if (mA == 0) {
    return 0;
  }
...
// void TMCBaseComponent::enable(bool enable, bool recover_toff = true) {
void TMCBaseComponent::enable(bool enable) {
  if (this->enn_pin_ != nullptr) {
